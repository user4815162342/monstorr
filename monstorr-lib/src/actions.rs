        ...
        match self {
          Action::ChangeShape(form) => 
            ...
            Action::IllusoryAppearance(discern_dc) => 
                format!("${{Subj}} covers ${{refpro}} and anything ${{subjpro}} is wearing or carrying with a magical illusion that makes ${{objpro}} look like another creature of ${{posspro}} general size and humanoid shape. The illusion ends if ${{subj}} takes a bonus action to end it or if ${{subjpro}} dies.${{par}}The changes wrought by this effect fail to hold up to physical inspection. Otherwise, a creature must take an action to visually inspect the illusion and succeed on a DC {} Intelligence (Investigation) check to discern that ${{subj}} is disguised.",discern_dc), 
            Action::Leadership => 
