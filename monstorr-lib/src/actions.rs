    ...
    pub fn get_description(&self) -> String {
        match self {
          Action::ChangeShape(form) => 
                format!("${{Subj}} magically polymorphs into {}, or back into its true form. It reverts to its true form if it dies. Any equipment it is wearing or carrying is absorbed or borne by the new form (${{poss}} choice). In a new form, ${{subj}} retains its alignment, hit points, Hit Dice, ability to speak, proficiencies, Legendary Resistance, lair actions, and Intelligence, Wisdom, and Charisma scores, as well as this action. Its statistics and capabilities are otherwise replaced by those of the new form, except any class features or legendary actions of that form.",form), 
            Action::Charm(distance,save_dc) => 
                format!("One humanoid ${{subj}} can see within {} feet of it must succeed on a DC {} Wisdom saving throw or be magically charmed for 1 day. The charmed target obeys ${{poss}} verbal or telepathic commands. If the target suffers any harm or receives a suicidal command, it can repeat the saving throw, ending the effect on a success. If the target successfully saves against the effect, or if the effect on it ends, the target is immune to this ${{name}}'s Charm for the next 24 hours.${{par}}${{Subj}} can have only one target charmed at a time. If it charms another, the effect on the previous target ends.",distance,save_dc), 
            Action::Enlarge => 
                format!("For 1 minute, ${{subj}} magically increases in size, along with anything it is wearing or carrying. While enlarged, ${{subj}} is Large, doubles its damage dice on Strength-based weapon attacks (included in the attacks), and makes Strength checks and Strength saving throws with advantage. If ${{subj}} lacks the room to become Large, it attains the maximum size possible in the space available."),
            Action::Etherealness => 
                format!("${{Subj}} enters the Ethereal Plane from the Material Plane, or vice versa. It is visible on the Material Plane while it is in the Border Ethereal, and vice versa, yet it can't affect or be affected by anything on the other plane."),
            Action::FrightfulPresence(distance,save_dc,resave_disadvantage) => 
                format!("Each creature of ${{poss}} choice that is within {} feet of ${{subj}} and aware of it must succeed on a DC {} Wisdom saving throw or become frightened for 1 minute. A creature can repeat the saving throw at the end of each of its turns, {}ending the effect on itself on a success. If a creature's saving throw is successful or the effect ends for it, the creature is immune to ${{poss}} Frightful Presence for the next 24 hours.",distance,save_dc,if *resave_disadvantage {"with disadvantage if ${{subj}} is within line of sight, "} else {""}),
            Action::HorrifyingVisage(distance,save_dc) => 
                format!("Each non-undead creature within {} ft. of ${{subj}} that can see it must succeed on a DC {} Wisdom saving throw or be frightened for 1 minute. If the save fails by 5 or more, the target also ages 1d4 x 10 years. A frightened target can repeat the saving throw at the end of each of its turns, ending the frightened condition on itself on a success. If a target's saving throw is successful or the effect ends for it, the target is immune to this ${{name}}'s Horrifying Visage for the next 24 hours. The aging effect can be reversed with a greater restoration spell, but only within 24 hours of it occurring.",distance,save_dc), 
            Action::IllusoryAppearance(discern_dc) => 
                format!("${{Subj}} covers itself and anything it is wearing or carrying with a magical illusion that makes it look like another creature of its general size and humanoid shape. The illusion ends if ${{subj}} takes a bonus action to end it or if it dies.${{par}}The changes wrought by this effect fail to hold up to physical inspection. Otherwise, a creature must take an action to visually inspect the illusion and succeed on a DC {} Intelligence (Investigation) check to discern that ${{subj}} is disguised.",discern_dc), 
            Action::Leadership => 
            ...
            Action::Reel => 
                format!("${{Subj}} pulls each creature grappled by it up to 25 ft. straight toward it."),
            Action::Swallow(damage,vomit_damage,vomit_save_dc,escape_distance) => 
                format!("${{Subj}} makes one bite attack against a Medium or smaller creature it is grappling. If the attack hits, that creature takes the bite's damage and is swallowed, and the grapple ends. While swallowed, the creature is blinded and restrained, it has total cover against attacks and other effects outside ${{subj}}, and it takes {} acid damage at the start of each of ${{poss}} turns.${{par}}If ${{subj}} takes {} damage or more on a single turn from a creature inside it, ${{subj}} must succeed on a DC {} Constitution saving throw at the end of that turn or regurgitate all swallowed creatures, which fall prone in a space within 10 feet oft he ${{name}}. If ${{subj}} dies, a swallowed creature is no longer restrained by it and can escape from the corpse using {} feet of movement, exiting prone.",damage,vomit_damage,vomit_save_dc,escape_distance), 
            Action::Teleport(distance,opt_attack) => 
                format!("${{Subj}} magically teleports, along with any equipment it is wearing or carrying, up to {} ft. to an unoccupied space it can see.{}",distance,if let Some(attack) = opt_attack {
...
impl LegendaryAction {
    ...
    pub fn get_main_description(amount: &u8) -> String {
        format!("${{Subj}} can take {} legendary actions, choosing from the options below. Only one legendary action option can be used at a time and only at the end of another creature's turn. ${{Subj}} regains spent legendary actions at the start of its turn.",amount)
    }
